...
This redirect rule will send all incoming links to `/foo` and `/foo.html` to `/bar`.
...
Finally, we run a global search for internal links to `/foo`, and make sure to adjust them to be `/nested/foo`.
